...
void nuevo_nodo();
void histograma_pk();
void matriz_Conect(); //contiene todos los vecinos del nodo i (tamaño: N x k_max  )   se usa en la funcion randomizar()
...
int main()
{
    ...
    printf("\n\nPrograma Growth & Play con Prob-payoff + Rand + %d jugadas extra (%d iteraciones)\nN=%d  b_ini=%lf b_fin=%lf eps=%lf  TauT=%d  TauD=%d\n", Niter,jugadas_extra,N,b_min,b_max,eps,tauT,tauD);
...
//Guarda la P(k) antes y despues de randomizar
	...
	for(iter=1;iter<=Niter;iter++)         //inicio bucle de estadistica
	{
	    ...
	    matriz_Conect();
	    matriz_D();                   //NO HACE FALTA CALCULAR LA MATRIZ C[i][j], basta con la M[i][j] pero la necesito para la funcion randomizar()
      ...
      if (cont==max)
        {
          ...
          if(GC<s)   //si red disconexa  OJO!!!! ANTES ERA <N, AHORA ES <s
           {
