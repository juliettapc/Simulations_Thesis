...
# define seleccion 1.0     // para la prob de cambio de estrategia de Fermi  (llamada w en las formulas)

# define  eps    1.0    // para la prob de attachment exp de la fitness




# define ratio_min   1.01     //cociente   beneficio/coste  (siempre >1)
# define ratio_max   6.01
...
int main()
{
   ...
   jugadas_extra=20000;
    ...
    sprintf(file4,"%dProp-top_%d-%d-ratio_ini%.2lf-e%.2lf_w%.2lf_%liextra_%diter_reasign.dat",N,tauT,tauD,ratio_min,eps,seleccion,jugadas_extra,Niter);   //un solo archivo     
...
/*	sprintf(file7,"%dPk%d-%d-ratio%.2lf-e%.2lf_size500_%diter.dat",N,tauT,tauD,ratio,eps,Niter); // para los P(k) durante el crecimiento
...
/*	sprintf(file777,"%dPk%d-%d-ratio%.2lf-e%.2lf_size2000_%diter.dat",N,tauT,tauD,ratio,eps,Niter);      
	fich777=fopen(file777,"wt");
	fclose(fich777);
	
	sprintf(file7777,"%dPk%d-%d-ratio%.2lf-e%.2lf_size4000_%diter.dat",N,tauT,tauD,ratio,eps,Niter);      
	fich7777=fopen(file7777,"wt");
	fclose(fich7777);
	
*/	
	
/*	sprintf(file6,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size500_w%.2f_%diter.dat",N,tauT,tauD,ratio,eps,seleccion,Niter); // para los P(k)c  durante el crecimiento
	fich6=fopen(file6,"wt");
	fclose(fich6); */     
	
	sprintf(file66,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size1000_w%.2f_%liextra_%diter_reasign.dat",N,tauT,tauD,ratio,eps,seleccion,jugadas_extra,Niter);      
	fich66=fopen(file66,"wt");
	fclose(fich66);      
	
/*	sprintf(file666,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size2000_w%.2f_%diter.dat",N,tauT,tauD,ratio,eps,seleccion,Niter);      
	fich666=fopen(file666,"wt");
	fclose(fich666);
	
	sprintf(file6666,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size4000_w%.2f_%diter.dat",N,tauT,tauD,ratio,eps,seleccion,Niter);      
...
/*	sprintf(file22,"%dCCk_%d-%d-ratio%.2lf-e%.2lf_w%.2f_%diter.dat",N,tauT,tauD,ratio,eps,seleccion,Niter);    // id.    
	...
	for(iter=1;iter<=Niter;iter++)         //inicio bucle de estadistica
	{
	    ...
	    //	      printf("aux=%d / %d = %f",n_coop,N,aux);


	      reasignar_ci();  // REASIGNANO LAS ESTRATEGIAS RESPETANDO LA <C>, luego en lugar de usar ro, en esta funcion usare aux=n_coop/(double)N;
...
void reasignar_ci ()
{
	...
	if(r<aux)   //aux=n_coop/N  para que de este modo se respete la <C> al randomizar
	{
	    e[i]=0;
	    n_coop++;
	}
