...
//18-6-08: calculo la Pkc(t) durante las jugadas extra, para ver como evoluciono (de <c> a <c>inf )
...
# define jugadas_extra  9999999      //despues de acabar de crecer la red
# define espaciado_temp  200      // una de cada cuantas veces miro el Pkc(t)  (DEBE SER:  jugadas_extra / espaciado_temp =100)
...
char file[256], file1[256], file4[256],file22[256],file21[256],file11[256];  
...
*/	
	...
	sprintf(file21,"%dNorma_%d-%d-b%.2lf-e%.2lf_%diter.dat",N,tauT,tauD,b,eps,Niter);    // archivo de normalizacion (uno para cada b)
	fich21=fopen(file21,"wt");
	fclose(fich21);
	...
	for(iter=1;iter<=Niter;iter++)         //inicio bucle de estadistica
	{
	    ...
	    sprintf(file11,"%dtpo_Nck_%d-%d-b%.2lf-e%.2lf_iter%d.dat",N,tauT,tauD,b,eps,iter);    // un archivo para cada b y cada realizacion
	    ...
	    // guardo el conteo de PK (sin normalizar)
	    
	    histograma_pk(); 

	    fich21=fopen(file21,"at");			  
	    fprintf(fich21,"%d     ", iter);   //guardo el numero de iteracion 	  	    
	    fclose (fich21);  
	    
	    
	    fich21=fopen(file21,"at");	
	    for(jj=1;jj<=K_MAX;jj++)
	    {  
		fprintf(fich21,"%lf     ", PK[jj]);     //guardo Nk  de cada iter (para luego normalizar las Pkc)	  	  
	    }
	    fprintf(fich21,"\n");  
	    fclose (fich21);    
		...
		if((ii % espaciado_temp) ==0.0)        // guardo cada x pasos de tiempo
		{
		    // printf("\ntpo %d",ii);
		    histograma_pk_coop();

		    fich11=fopen(file11,"at");			  
		    fprintf(fich11,"%d     ", ii);   //guardo el paso de tiempo 	  	    
		    fclose (fich11);  
		    ...
		    for(jj=1;jj<=K_MAX;jj++)
		    {  
			fprintf(fich11,"%lf     ", PK_coop[jj]);     //guardo la coop_media de cada iter	  	  
		    }
		    fprintf(fich11,"\n ");  
		    fclose (fich11);     
