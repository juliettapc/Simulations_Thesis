...
double PK[N+1], PK_tot[N+1], PKa[N+1], PKa_tot[N+1];    //  para la distribucion P(k)
double r,v;                     //para guardar los aleatorios
int  II,III,n,i,ii,j,jj,g,gg, d, q,y,z, x[N+1], C[N+1][K_MAX+1], steps;
double norma,norma2;
...
int main()
{
  ...
  */
    ...
    sprintf(file3,"%dC_med%d-%d-b%.2lf-e%.2lf.dat",N,tauT,tauD,b,eps);        
	...
	printf("\n\nPrograma Growth & Play  (%d iteraciones)\nN=%d  b=%lf eps=%lf  TauT=%d  TauD=%d\n", Niter,N,b,eps,tauT,tauD);
	...
	do{
	  ...
	  for(III=1;III<=tauT;III++)
	    {
	      
	      s++;	    
	      nuevo_nodo();	 
	      //printf("nuevo nodo: %d s:%d \n", III,s);
	      //printf("%lf\n",norma);	
//	    matriz_Conect();
	    }
...
void nucleo_inicial()    //conecto los m_o nodos iniciales todos entre si
{
  ...
  { 
      ...
      for(j=1; j<=i; j++)
      {                              
	  P[i] = P[i] + k[j]*fit[j];        //para los m_o fit[i]=1
      }
  ...
  for(q = 1; q<=m; q++)        //bucle a los m links nuevos que voy a añadir
    {
      ...
      for(j=g+1; j<s; j++)  
      {
	  P_prov[j]=P_prov[j]-k[g]*fit[g] ; 
      }
      
      norma2=norma2-k[g]*fit[g];
