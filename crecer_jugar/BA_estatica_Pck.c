...
//30-9-08: basado en el juego_general_k-new.c añadiendo calculo de Pck
...
# define jugadas_equil 3    //pasos de tiempo para hacer la media y discriminar


# define Niter 500        //estadistica
...
char nombre[256],nombre3[256],file7[256],file8[256],file9[256],file66[256],file77[256], nombre4[256];
...
double PK[N+1], PK_tot[N+1],PK_coop[N+1], PK_coop_tot[N+1];        //  para la distribucion P(k) y Pck
...
FILE *escribe2;
FILE *escribe3;
FILE *fich7,*fich8,*fich9,*fich66,*fich77,*fich10,*fich11, *escribe4;



double b=1.00,delta_b=0.2;
...
int main()
{
  ...
  sprintf(file7,"Clusters_b%.2lf-%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.dat",b,b+nb*delta_b,alfa,Su,Pu,N,Niter);
  fich7=fopen(file7,"wt");
  fclose(fich7);
  
  //Guarda <c>, CP,DP y  F : un solo fichero (pq ahora no barrermos en rho)
  sprintf(nombre3,"fluct_b%.2lf-%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.dat",b,b+(nb-1)*delta_b,alfa,Su,Pu,N,Niter);    
...
/*  sprintf(nombre4,"rho_vs_k_b%.2lf-%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.dat",b,b+(nb-1)*delta_b,alfa,Su,Pu,N,Niter);    
  ...
  for(ib=0;ib<nb;ib++)     //bucle externo para barrer en b
	...
	escribe2=fopen("comprobar.dat","wt");*/
     	...
     	sprintf(file66,"%dPkcb%.2lf_%dtransit_%diter.dat",N,b,jugadas,Niter);      
	fich66=fopen(file66,"wt");
	fclose(fich66);       
      

	sprintf(file77,"%dPk_b%.2lf__%diter.dat",N,b,Niter);      
...
/*	  sprintf(file8,"Histogr_Times_b%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.dat",b,alfa,Su,Pu,N,Niter);
	  fich8=fopen(file8,"wt");
	  fclose(fich8);*/	
	  
          //Guarda la Distribucion de Frecuencias versus K: un histogr. para cada valor de rho y de b
	    /*  sprintf(file9,"Frecs_k_b%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.dat",b,alfa,Su,Pu,N,Niter);
 ...
 for(i = 1; i <= N; i++)         //acumulo
	...
	fich66=fopen(file66,"at");     // Escribo PKc tras el transitorio (jugadas)      
	for(i = 1; i<=N; i++)         
	{	 
	    if(PK_tot[i]!=0.0)
	    {  	    	    
		fprintf(fich66,"%d   %d   %lf\n", jugadas,i,PK_coop_tot[i]/PK_tot[i]);
	    }



	    /*    else            OJO!!!! esta intruccion falsearia los valores Pkc=0, confundiendo cuando Pk=0 con cuando Pk!=0  
	    {                             me interesa representar con Pkc=0  sólo los casos en los que Pk!=0         
 

		fprintf(fich66,"%d   %d   0.000\n", jugadas_extra,i);
		}*/             
...
fich77=fopen(file77,"at");     // Escribo PK  de s=1000      
   ...
   /*   fich9=fopen(file9,"at");
      ...
      for(i=1;i<=jugadas_equil;i++)              
	{
	  ...
	  fprintf(fich8,"%i   %lf   %lf   %lf   %lf\n",i,Tiempos[i],VentanasCoop[i],b,ro);
      ...
      else 
	{
      ...
      for (i=1;i<=K_MAX;i++)
	{
	  fprintf(escribe4,"%f   %d   %lf   %lf   %lf  \n",
		  b,i, rho_C_k[i], rho_D_k[i], rho_Fl_k[i]);
	}
      fprintf(escribe4,"\n");
      fclose(escribe4);	*/
...
for(n=1;n<=jugadas;n++)            //pasos de tiempo
 ...
 for (i=1;i<=N;i++)      //actualizacion de estrategias en paralelo
      {
	e[i]=e_aux[i];
      }
