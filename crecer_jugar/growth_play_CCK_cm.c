...
//calculo del histograma del clust_coef en funcion de k y tb del histograma de cooperadores

//eleccion del carácter inicial del nuevo nodo añadido con prob=<c>(t)
...
/*  sprintf(file3,"%dC_med%d-%d-b_ini%.2lf-e%.2lf_%diter.dat",N,tauT,tauD,b_min,eps,Niter);   //un solo archivo     
  ...
  sprintf(file4,"%dProp-top_%d-%d-b_ini%.2lf-e%.2lf_%diter_cm.dat",N,tauT,tauD,b_min,eps,Niter);   //un solo archivo     
  ...
  while(b<=b_max)     //bucle externo para barrer en b
    {
      ...
      sprintf(file7,"%dPk%d-%d-b%.2lf-e%.2lf_%diter_cm.dat",N,tauT,tauD,b,eps,Niter);   //un p(k) para cada b     
      fich7=fopen(file7,"wt");
      fclose(fich7);

      sprintf(file77,"%dCk%d-%d-b%.2lf-e%.2lf_%diter_cm.dat",N,tauT,tauD,b,eps,Niter);   //un p(k) para cada b     
      fich77=fopen(file77,"wt");
      fclose(fich77);

      sprintf(file,"%dEvolCoop%d-%d-b%.2lf-e%.2lf_%diter_cm.dat",N,tauT,tauD,b,eps,Niter);    // id.    
      fich=fopen(file,"wt");
      fclose(fich);

      sprintf(file1,"%dnum_coop%d-%d-b%.2lf-e%.2lf_%diter_cm.dat",N,tauT,tauD,b,eps,Niter);    // id.    
...
void nuevo_nodo() 
{
 ...
 double aux, aux1, prob_caracter;
    ...
    {	
	//tamaño actual del sistema:  s=N(t)
	aux=n_coop;
	aux1=s;   //no cuento el propio nodo que acabo de añadir 
	
	prob_caracter=aux/(aux1-1.0);            // establezco el carcter del nuevo nodo:  sera cooperador con prob= <c>(t)  de la red en total
	r= FRANDOM;
	...
	//printf("tamaño sist.:%d, <C>:%lf   r:%lf \n\n",s,prob_caracter,r);
