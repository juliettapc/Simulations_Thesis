...
int main()
{
 ...
 sprintf(file4,"%dProp-top_%d-%d-ratio_ini%.2lf-e%.2lf_w%.2lf_%liextra_%diter_rand_red_y_ci50.dat",N,tauT,tauD,ratio_min,eps,seleccion,jugadas_extra,Niter);   //un solo archivo     
    ...
    printf("\n\nPrograma Growth & Play con Prob.Fermi+Attch.Exp y Matriz Payoff en funcion de ratio==b/c + Randomizar la red + c.i. al 50(%d iteraciones)\nN=%d  ratio_ini=%lf ratio_fin=%lf  eps=%.2lf  w=%f  TauT=%d  TauD=%d\n\n", Niter,N,ratio_min,ratio_max,eps,seleccion,tauT,tauD);
