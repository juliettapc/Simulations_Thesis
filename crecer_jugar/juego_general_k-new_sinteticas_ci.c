...
// 30-6-08: en lugar de usar una sola vez cada red (tengo 100), hago con cada una 10 realizaciones con c.i. random al 50%
...
# define Niter  2      //estadistica   (numero de ficheros de redes que tengo =100)
# define realizaciones  5   // con c.i. al 50% random, pero la misma red
...
char nombre[256],nombre3[256],file7[256], nombre4[256],file3[256],file77[256];
...
FILE *escribe2;
FILE *escribe3;
FILE *fich7,*fich10,*fich11, *escribe4,*fich3,*fich77;
...
int main()
{
  ...
  printf("con Red SF Sintetica (2.7) de N=%d (%d iteraciones(redes) y %d realizaciones con cada red)\n",N,Niter,realizaciones);
  printf("S=%lf P=%lf \n", Su, Pu);  


  //Guarda los Clusters de Cooperantes etc.
  sprintf(file7,"Clusters_b%.2lf-%.2lf_N%d_%diter_ci.dat",b,b+nb*delta_b,N,Niter);
  fich7=fopen(file7,"wt");
  fclose(fich7);
  
  //Guarda <c>, CP,DP y  F : un solo fichero (pq ahora no barrermos en rho)
  sprintf(nombre3,"fluct_b%.2lf-%.2lf_sintetic_N%d_%diter_ci.dat",b,b+(nb-1)*delta_b,N,Niter);    
 ...
 //Guarda la densidad de coop puros, defect puros y fluct en funcion de la conectividad : un solo fichero 
  sprintf(nombre4,"rho_vs_k_b%.2lf-%.2lf_N%d_%diter_ci.dat",b,b+(nb-1)*delta_b,N,Niter);    
...
//bucle para hacer varias realizaciones con la misma red
	      
	      for (rr=1;rr<=realizaciones;rr++)
	      {
		  printf("   realizacion:%d\n",rr);
		  ...
		  //printf("recuento estrategias post transitorio hecho\n");
	      ...
	      }//fin bucle de realizaciones sobre la misma red
	      
	    }       //fin bucle estadística
	  
	  
	  
	  c_media=c_media/(double)(Niter*N*realizaciones);            //normalizar, y tanto por uno
	  
	  cp_media=cp_media/(double)(Niter*N*realizaciones); 
	  dp_media=dp_media/(double)(Niter*N*realizaciones); 
	  fl_media=fl_media/(double)(Niter*N*realizaciones);
...
//printf("realizados cambios estrat\n");
...
//printf("hecho recuento coop y normalización\n");	
 //discriminacion:
...
//printf("fin de la discriminacion\n");	
