...
// oooooojjjjjjooooooo!!!!   comoprobar si estan comentados los archivos para las Pk de todos los tamaños!!!

//21-7-08: en esta version me interesa tener la Pck (t)  de distintos instantes de tiempo durante las jugadas extra


//27-8-08:  al acabar de crecer la red, reasigno estrategias al 50%, dejo evolucionar y luego mido Pck  (basdo en la reasignacion hecha para la comparacion de Cinf del EPA


//2-9-08:   AHORA, AL ACABAR DE JUGAR,  ME INTERESA REASIGNAR LAS ESTRATEGIAS PERO RESPETANDO LA <C>, Y LUEGO HACER LAS JUGADAS EXTRA
...
# define jugadas_extra   17000       //despues de acabar de crecer la red y reasignar las estrategias RESPETANDO LA <C>
...
int main()
{
    ...
    printf("\n\nPrograma Growth & Play con Probabilidad-payoff + Reasig estrat (misma <C>)+ %d jugadas extra (%d iteraciones)\nN=%d  b_ini=%lf b_fin=%lf eps=%lf  TauT=%d  TauD=%d  %d jugadas extra\n",jugadas_extra,Niter,N,b_min,b_max,eps,tauT,tauD,jugadas_extra);
	...
	sprintf(file66,"%dPkc%d-%d-b%.2lf-e%.2lf_size1000_%diter_%dextra.dat",N,tauT,tauD,b,eps,Niter,jugadas_extra);      
	...
	for(iter=1;iter<=Niter;iter++)         //inicio bucle de estadistica
	{
	    ...
	    reasignar_ci();  // AHORA ME INTERESA REASIGNAR LAS ESTRATEGIAS PERO RESPETANDO LA <C>, luego en lugar de usar ro, en esta funcion usare aux=n_coop/(double)N;
...
// Escribo y normalizo (por el numero de iteraciones) las PK de coop 
	...
	for(i = 1; i<N; i++)         
	{	 
	    if(PK_tot_1000[i]!=0.0)
	    {  	    	    
		fprintf(fich66,"%d   %d   %lf\n", jugadas_extra,i,PK_coop_tot_1000[i]/PK_tot_1000[i]);
	    }
