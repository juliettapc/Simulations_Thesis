...
void matriz_D(); //contiene todos los pares de vecinos (tama�o real: ((N-m_o)*m+3) x 2  )
void randomizar();
void check_GC();
void reasignar_ci ();
...
int main()
{
    ...
    sprintf(file4,"%dProp-top_%d-%d-ratio_ini%.2lf-e%.2lf_w%.2lf_%liextra_%diter_rand-reasign.dat",N,tauT,tauD,ratio_min,eps,seleccion,jugadas_extra,Niter);   //un solo archivo     
	...
	sprintf(file66,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size1000_w%.2f_%liextra_%diter_rand-reasign.dat",N,tauT,tauD,ratio,eps,seleccion,jugadas_extra,Niter);      
...
void randomizar()
{
  ...
  check_GC();      //guardo el tamaño de referencia que tiene la red antes de randomizarla
 tamagno_real=GC;
