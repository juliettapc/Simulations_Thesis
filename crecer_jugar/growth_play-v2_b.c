...
//incluye bucle de estadística y de barrido en b
...
int main()
{
  ...
  sprintf(file3,"%dC_med%d-%d-b_ini%.2lf-e%.2lf.dat",N,tauT,tauD,b,eps);   //un solo archivo     
  ...
  while(b<=b_max+delta_b)     //bucle externo para barrer en b
    {
      printf("\nb=%lf\n",b);
      ...
      sprintf(file7,"%dPk%d-%d-b%.2lf-e%.2lf.dat",N,tauT,tauD,b,eps);   //un p(k) para cada b     
  fich7=fopen(file7,"wt");
  fclose(fich7);
  
  sprintf(file,"%dEvolCoop%d-%d-b%.2lf-e%.2lf.dat",N,tauT,tauD,b,eps);    // id.    
  fich=fopen(file,"wt");
  fclose(fich);
