...
# define seleccion  0.1     // para la prob de cambio de estrategia de Fermi  (llamada w en las formulas)


# define  eps    0.10    // para la prob de attachment exp de la fitness
...
int main()
{
  ...
  jugadas_extra=20000;
    ...
    sprintf(file4,"%dProp-top_%d-%d-ratio_ini%.2lf-e%.2lf_w%.2lf_%liextra_%diter.dat",N,tauT,tauD,ratio_min,eps,seleccion,jugadas_extra,Niter);   //un solo archivo     
...
/*	sprintf(file6,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size500_w%.2f_%liextra_%diter.dat",N,tauT,tauD,ratio,eps,seleccion,jugadas_extra,Niter); // para los P(k)c  durante el crecimiento
	fich6=fopen(file6,"wt");
	fclose(fich6);  */    
	


	sprintf(file66,"%dPkc%d-%d-ratio%.2lf-e%.2lf_size1000_w%.2f_%liextra_%diter.dat",N,tauT,tauD,ratio,eps,seleccion,jugadas_extra,Niter);
