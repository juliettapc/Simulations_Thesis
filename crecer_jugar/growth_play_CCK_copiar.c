...
//calculo del histograma del clust_coef en funcion de k, y tb histograma de cooperadores

//eleccion del carácter inicial del nuevo nodo añadido: copiar al vecino que mas gane
...
# define  tauT 10      //(si =10, pongo 10 nodos y juego una vez)
# define  tauD 1
...
int NCLUSTERS,GComp,warning[N+1],analyse[N+1],kk,I,J;
...
/*  sprintf(file3,"%dC_med%d-%d-b_ini%.2lf-e%.2lf_%diter.dat",N,tauT,tauD,b_min,eps,Niter);   //un solo archivo     
  ...
  sprintf(file4,"%dProp-top_%d-%d-b_ini%.2lf-e%.2lf_%diter_copiar.dat",N,tauT,tauD,b_min,eps,Niter);   //un solo archivo     
  ...
  while(b<=b_max)     //bucle externo para barrer en b
    {
      ...
      sprintf(file7,"%dPk%d-%d-b%.2lf-e%.2lf_%diter_copiar.dat",N,tauT,tauD,b,eps,Niter);   //un p(k) para cada b     
      fich7=fopen(file7,"wt");
      fclose(fich7);

      sprintf(file77,"%dCk%d-%d-b%.2lf-e%.2lf_%diter_copiar.dat",N,tauT,tauD,b,eps,Niter);   //un p(k) para cada b     
      fich77=fopen(file77,"wt");
      fclose(fich77);

      sprintf(file,"%dEvolCoop%d-%d-b%.2lf-e%.2lf_%diter_copiar.dat",N,tauT,tauD,b,eps,Niter);    // id.    
      fich=fopen(file,"wt");
      fclose(fich);

      sprintf(file1,"%dnum_coop%d-%d-b%.2lf-e%.2lf_%diter_copiar.dat",N,tauT,tauD,b,eps,Niter);    // id.    
...
void nuevo_nodo() 
{
    ...
    int caracter_vecinos[m+1];
    int vecinos[m+1],aux,aux1;
  ...
  for(i=1;i<=m;i++)
  {
      vecinos[i]=0;      //guardo los m vecinos del nodo recien añadido
      caracter_vecinos[i]=1;      //los inicializo a defectores      
  }
  ...
  for(q = 1; q<=m; q++)        //bucle a los m links nuevos que voy a añadir
    {
      ...
      vecinos[q]=g;      //guardo  el vecino del nodo recien añadido
      
      if( e[g]==0 )      //si el vecino es cooperador
      {
	  caracter_vecinos[q]=0;
      }
  ...
  {  
      aux1= vecinos[i];
      //printf("vecino:%d  ben:%lf ",aux1,ben[aux1]);
  }
  ...
  if (s<30)    //para evitar que salga todo cooperacion, el caracter de los primeros es aleatorio
  {
      r=FRANDOM;
  ...
  else      // establezco el carcter del nuevo nodo:una vez lanzados sus links, copiara al vecino que mas gane
  {
      max_ben=0.0;
      for(i=1;i<=m;i++)
      {
      ...
      e[s]=e[copiar];  //copio estrategia del vecino que mas gana      
/*printf("elijo vecino:%d ",copiar);
  printf("\n");*/
  }
