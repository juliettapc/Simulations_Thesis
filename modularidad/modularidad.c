...
# define T_max    0.00900       //temperatura para el SA:     si M-M_old son del orden de 0.0005
# define T_min    0.0            // entonces, T_max: 0.009 
# define delta_T  0.00050      // (para que la exp haga que la prob. de cambio a peor 
                               //   sea solo de vez en cuando) y delta_T:0.000045
...
{     
  ...
  printf("\nPrograma para obtener la estructura de comunidades de una red \n maximizando la modularidad por SA.   T_max:%f - T_min:%f  (delta_T:%f)\n\n",T_max,T_min,delta_T);
  ...
  /////////archivo de salida
  sprintf(nombre2,"Comunid_N%d_Nlinks%d_T%f-%f.dat",N,filas,T_max,T_min); 
  ...
  printf("T:%f  (t:%d)  M_ini:%f   NUmCom:%d\n\n",T,tpo,M_old,NumCom);
  
  while(T>T_min)   //inicio bucle Simulated Annealing  -SA-
    {            
      printf("\nT:%f\n",T); 
	  ...
	  if(prob>N)     ////////////cambio indiv.
	    {
	      ...
	      cambio_indiv(nodo_elegido);     //modifica el indice:   nodo_aux[nodo_elegido]
	      
	      
	      
	      M=calculo_modularidad();  //de la particion nueva  
	      
	      //	      printf("T:%f   M:%f  M_old:%f   NumCom:%d  ",T,M,M_old,NumCom);
	      ...
	      else   //acepto el cambio con una peq. prob. (para escapar de max. locales)
		{
		  ...
		  if(prob<exponencial)
		    {
		      nodo[nodo_elegido]=nodo_aux[nodo_elegido]; //acepto el cambio 
		      //printf("  -->   acepto cambio indiv (prob.:%f < exp:%f)\n",prob,exponencial);
...
void escindir()     //elijo una comunidad y la parto en dos (haciendole un SA anidado)
{
  ...
  for(i=1;i<=N;i++)     //obtengo el tamaño de la comunidad en cuestion
    {            //y la parto en dos aleatoriamente
      ...
      nodo_escin[i]=0;  // =comunidad1  o  =comunidad2:  el nodo pertenece a una comunidad. o la otra, 
                        //   =0 a ninguna de las dos

      comunidad[i]=0;

      if(nodo[i]==comunidad1)   //si el nodo pertenecia a la comunidad que kiero escindir
	{
	  size++;
  ...
  for(f=1;f<=size;f++)       //recorro solo los nodos de la comunidad escindida
    {
      ...
      links_inter_intra[nodo_aux_escin[nodo1]]+=k[nodo1];//calculo conectividad inter+intra 
    }                                  //sumada a los nodos de las dos comunidades
  


  num_links_anidado=0;    //num. tot. links de la comunidad a escindri
  for(f=1;f<=filas;f++)   //calculo la suma de links inter de cada comunidad
    {
      nodo1=D[f][1];
      nodo2=D[f][2];
	...
	{   // sólo si alguno de los dos pertenece a la comunidad (OJO! INDICE ORIGINAL DE LA RED ENTERA!!)
	  ...
	  if(nodo_aux_escin[nodo1] == nodo_aux_escin[nodo2])    
	    {          //si ambos pertenecen a la comunidad  INDICE RELATIVO AL SA ANIDADO
