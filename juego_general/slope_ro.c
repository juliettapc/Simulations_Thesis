     /*Implementacion del Dilema del Prisionero en una red gardeñes-moreno*/
                   //contiene ademas un bucle de estadística y otro para barrer en b y obtener el n_coop en le equil. , usando el mecanismo de parada
...
# define Njugadas 5000  //pasos de tiempo para el transitorio
# define Niter 1   //estadistica
# define ventana 5000
# define Nventanas 5000
...
int  e[N+1],  w, t, k_m;       // variables para el dilema
double  ben[N+1],p;

double ro, ro_max, ro_min, delta_ro, c_media;                 //barrer en ro y estadistica
int iter;

double c_instant[ventana+1],n_coop;                 //para determinar el equilibrio
int I, II,a;
double s1,s2,s3,s4,dobleprec;
double pendiente, umbral;
...
umbral=(double)N*0.015/(double)ventana;     // permito una variacion del 0.15%

/*sprintf(nombre,"pk_gm_N%d_alfa%.3f.dat",N,alfa);
escribe=fopen(nombre,"wt");
escribe2=fopen("comprobar.dat","wt");*/

sprintf(nombre3,"slope_c_med_frente_ro%.2lf-%.2lf_N%d_alfa%.3f_b%.3f.dat",ro_min,ro_max,N,alfa,b);

escribe3=fopen(nombre3,"wt");    

printf("\n\n\nPrograma Slope_ro\n"); 
printf("Red de N=%d, con alfa=%lf y  b=%lf\n",N,alfa,b);
 ...
 while(ro<=ro_max)
{
 ...
 c_media=0;
  for(iter=1;iter<=Niter;iter++)          //estadistica
             
  {
   ...
   n_coop=0;
   for(I=1;I<=Nventanas;I++)
   {
     s1=s2=s3=s4=0;

     for(II=1;II<=ventana;II++)
      {
	  c_instant[II]=0;
      }
     ...
     {
	 dobleprec=a;
	 s1+=c_instant[a]*dobleprec;
	 s2+=dobleprec;
	 s3+=c_instant[a];
	 s4+=dobleprec*dobleprec;
     }
     dobleprec=ventana;
     pendiente=(s1*dobleprec-s2*s3);
     pendiente=pendiente/(dobleprec*s4-s2*s2);
//printf("pend:%lf\n",pendiente);
   
   if (fabs(pendiente)<=umbral)
       {
	   printf("umbral alcanzado \n");
	    break;
	}
  
   }  //fin bucle a Nventanas

    n_coop=s3/((double)ventana);
    //printf("n_coop:%lf\n",n_coop);
    c_media+=n_coop;
 ...
 dobleprec=Niter;
 dobleprec=dobleprec*N;
 c_media=c_media/dobleprec;           //normalizo y tanto por uno


printf("ro= %lf  c_media:%lf\n",ro,c_media);


fprintf(escribe3,"%lf  %lf\n", ro,c_media);
fflush(escribe3);          //bolcar los datos del buffer
...
for(n=1;n<=Njugadas;n++)            //pasos de tiempo
   {
    n_coop=0;
...
void juego2()            //hacia el equilibrio (ventanas)
{
...
for(n=1;n<=ventana;n++)            //pasos de tiempo
   {
    c_instant[n]=0;
