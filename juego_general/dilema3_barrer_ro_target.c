...
//obtencion de la dependencia de la concentracion de cooperantes en el equilibrio
//con  la concentracion inicial de cooperantes (distribuidos segun su conectividad)
...
int  i, j, jj, w,g,d, q, x[N+1],y,z, C[N+1][K_MAX+1],n, ValorA,steps,s,flag;
...
double  ben[N+1],p;
double ro, ro_min, ro_max, delta_ro, c_media;
int iter;
...
sprintf(nombre3,"ro%.2lf-%.2lf_b%.2lf_mas_conect.dat",ro_min,ro_max,b);
# endif

# ifdef menos_conectados
sprintf(nombre3,"ro%.2lf-%.2lf_b%.2lf_menos_conect.dat",ro_min,ro_max,b);
# endif
...
for(i=1;i<=N;i++)
   {
    ben[i]=0;
    e[i]=1;        //los inicializo todos a defectores con beneficio 0
   }
