...
# define b 1.9
# define jugadas 20000  //pasos de tiempo para el juego
...
int   M[N+1][N+1], unido[m+1],Cuidado[N+1];              //  unido[] guarda a quién se ha unido i , y cuidado[] quien le ha lanzado ya un link
...
double  ben[N+1],p,dobleN,dobleprec,ro;
...
//escribe2=fopen("comprobar.dat","wt");
    sprintf(nombre3,"dilema_b%.2lf_N%d_alfa%.3f_ro%.3f.dat",b,N,alfa,ro);
...
for(i=1;i<=N;i++)           // establezco inicialmente el 50% de cooperantes
 ...
 dobleprec=n_coop;
 dobleprec=dobleprec/dobleN;

fprintf(escribe3,"0     %lf\n",dobleprec);
//printf("num coop.: %d  \n",  n_coop);
...
//  printf("Defectores persistentes: \n");
// for(i=1;i<=N;i++)     //estudio defectores finales
//    {
//    if(e[i]==1)
//      {
//       printf("\n");
//      printf("\n El nodo %d tiene %d vecinos, de tipo:   ",i, k[i]);
//      for(j=1;j<=k[i];j++)
//         printf("%d    ",e[C[i][j]]);
//       }
