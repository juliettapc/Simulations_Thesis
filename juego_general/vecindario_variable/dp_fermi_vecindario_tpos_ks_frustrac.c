...
# define ratio_min    5.21     //cociente   beneficio/coste  (siempre >1)
# define ratio_max    5.22
...
# define KSTAR     100  // numero de vecinos con los que juego/me comparo (entre m y N)
...
int tpo_fijac;
double media_tpo_fijac;
double sigma_2_tpo,resta_2_tpo,tpo_f[Niter+1],aux;
...
int main()
{
    ...
    printf("\n\n\nPrograma PD con vecindad variable (frustracion + simetria)\n\n"); 
    ...
    sprintf(nombre3,"v__C_med_ratio%.2lf-%.2lf_alfa%.1lf_selecc%.2lf_kstar%d_N%d_%diter_frustr.dat",ratio_min,ratio_max,alfa,seleccion,KSTAR,N,Niter); 
	...
	sprintf(nombre2,"v__Evoluc_coop_ratio%.2lf_alfa%.1lf_selecc%.2lf_N%d_%diter_frustr.dat",ratio,alfa,seleccion,N,Niter); 
	...
	for(iter=1;iter<=Niter;iter++)
	{
	 ...
	 media_tpo_fijac=media_tpo_fijac/Niter;



	for(iter=1;iter<=Niter;iter++)      //calculo la desviacion estandar de los tiempos de consenso y de la S_max  
	{
	    resta_2_tpo=tpo_f[iter]-media_tpo_fijac;
	...
	printf("\nb/c=%lf c_media:%f  media_tpo_fijac:%f (error:%f)\n",ratio ,c_media,media_tpo_fijac,sigma_2_tpo); 
	...
	fprintf(escribe3,"%f  %f  %f  %f  %d  %f\n",ratio,c_media,media_tpo_fijac,sigma_2_tpo,KSTAR,seleccion);      
	fclose(escribe3);
...
//printf("\n\n\njugada:  %d,",tpo);
	...
	if(n_coop==0 || n_coop==N )//si  alacanza el estado absorbente antes de que pasen todas las jugadas
	{
	    tpo_f[iter]=tpo;
	    media_tpo_fijac+=tpo;
