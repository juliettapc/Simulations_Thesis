...
# define N 1100 //tamaño de la red
...
double normat,normaVentanasCoop,FREC[N+1], FRECMED,frecmed[K_MAX+1] , max;
...
int  kk;         //de prueba, todos enteros (sólo lo era kk)
...
int main()
{
    ...
    ro_min=0.5;      //asi no barre en concentracion inicial (bucle trivial)
    ro_max=0.51;
    delta_ro=0.1;
    b=T;
    delta_b=0.1;
...
/*sprintf(nombre,"pk_gm_N%d_alfa%.3lf.dat",N,alfa);
  ...
  escribe2=fopen("comprobar.dat","wt");*/
    ...
    sprintf(nombre3,"fluct_ro%.2lf-%.2lf_b%.2lf_alfa%.1lf_N%d_%diter1.dat",ro_min,ro_max,b,alfa,N,Niter);    
    escribe3=fopen(nombre3,"wt");   
    
//Guarda la Transicion de Cooperantes y otras cosas
    sprintf(file7,"Transicion_N%i_alfa%.2lf-bini%.2lf_ro%.2lf-%.2lf.dat",N,alfa,T,ro_min,ro_max);
...
/* comprobar=time(0);
   printf("time0= %f \n",comprobar);*/
    ...
    for(ib=0;ib<nb;ib++)      //bucle para barrer en b
    {
...
//Guarda la Distribucion de Tiempos de Cooperacion  (un fichero para cada b y cada rho)
 sprintf(file8,"Histogr_Times_N%i_alfa%.2lf-b%.2lf_ro%.2lf.dat",N,alfa,T,ro);
 fich8=fopen(file8,"wt");
 fclose(fich8);	

//Guarda la Distribucion de Frecuencias versus K  (un fichero para cada b y cada rho)
 sprintf(file9,"Frecs_k_N%i_alfa%.2lf-b%.2lf_ro%.2lf.dat",N,alfa,T,ro);
 ...
 fclose(fich9);
	...
	for(iter=1;iter<=Niter;iter++)
	{
	    ...
	    if(marca==1)
	    {
		//printf("antes   juego cp=%d dp=%d\n",cp,dp);//CONTROL
		
		juego_equil();    //  jugadas_equil 
		
		//printf("despues juego cp=%d dp=%d\n",cp,dp);//CONTROL
...
//   ( calculos de histogramas tiempos al final de la funcion juego_equil )
	...
	{
	    Tiempos[i]=Tiempos[i]/normat;   //histograma tiempos coop. tot
...
//CORRECCION en la normalización: diferencia máxima de coef de la matriz de payoff*max k (antes: b*k_m siempre)
 ...
 {                  //  usarla para discriminar   ( S[i] = Marcador[i] )
     
     S[i]=-1;   //todos como fluct             	
...
//CORRECCION en la normalización: diferencia máxima de coef de la matriz de payoff*max k (antes: b*k_m siempre)
 ...
 //discriminacion:
	...
	for(i=1;i<=N;i++)         
	{
	    ...
	    if(e[i]!=Estado[i][0])    //si ha cambiado
	    {
	    ...
	    if(S[i]==0)        //si era coop puro
	    {
		if(e[i]==1)          //y ahora ha sido defect
		{
		    //Pvar[k[i]]++;
		    S[i]=-1;    //fluctuante pa siempre 
		}
	    }
	    
	    if(S[i]==1)            //si era defect puro
	    {
		if(e[i]==0)        //y ahora ha sido coop
		{
		    S[i]=-1;        //fluctuante pa siempre
		    //Pvar[k[i]]++;
		}
    ...
    for(i=1;i<=N;i++)  //recuento numero cooperadores puros etc
	{
	    if(S[i]==0){
		cp++;
		//printf("entro a cambiar cp\n");
	    }

	    else 
		if(S[i]==1){
		    dp++;
		    //printf("entro a cambiar dp\n");
		}
		else{
		    fl++;	    
		    //printf("entro a cambiar fl  S[i]=%d\n",S[i]);
		}
