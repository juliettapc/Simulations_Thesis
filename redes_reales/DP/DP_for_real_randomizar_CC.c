...
# define Niter 20      //estadistica  (numero de ci distintas con para implementar la dinamica)
...
char nombre1[256],nombre2[256],nombre3[256],nombre4[256],nombre5[256],nombre6[256],nombre7[256];
...
//sprintf(nombre6,"CCk_email.txt");  
    //     sprintf(nombre6,"CCk_SCN.txt");     
     //   sprintf(nombre6,"CCk_jazz.txt");   
         sprintf(nombre6,"CCk_ASTRO.txt");  
  
    archivo6=fopen(nombre6,"wt");
    fclose(archivo6);
...
//    fclose(archivo4);
    ...
    clustering_coef();    //calcula tb CCk
   cl_coef_rand=0;


   archivo6=fopen(nombre6,"at"); 
   for(ii=1;ii<=N;ii++)
   { 
       fprintf(archivo6,"%d  %lf\n",ii,CC_k[ii]);          
   }
   fclose (archivo6);
	...
	// sprintf(nombre7,"CCk_email_b%.2f_%diter_rand.txt",b,Niter);  
	// sprintf(nombre7,"CCk_SCN_b%.2f_%diter_rand.txt",b,Niter);     
     //   sprintf(nombre7,"CCk_jazz_b%.2f_%diter_rand.txt",b,Niter);   
	sprintf(nombre7,"CCk_ASTRO_b%.2f_%diter_rand.txt",b,Niter);  
