...
# define N     1133      //13259astro    //198jazz     //12722SNC      //1133email //tamaño de la red
...
#define nb 35    //numero de valores de b
...
int main()
{
  ...
  sprintf(file7,"Clusters_b%.2lf-%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.txt",b,b+nb*delta_b,alfa,Su,Pu,N,Niter);
  fich7=fopen(file7,"wt");
  fclose(fich7);
  
  //Guarda <c>, CP,DP y  F : un solo fichero (pq ahora no barrermos en rho)
  sprintf(nombre3,"fluct_b%.2lf-%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.txt",b,b+(nb-1)*delta_b,alfa,Su,Pu,N,Niter);    
 ...
 //Guarda la densidad de coop puros, defect puros y fluct en funcion de la conectividad : un solo fichero 
  sprintf(nombre4,"rho_vs_k_b%.2lf-%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.txt",b,b+(nb-1)*delta_b,alfa,Su,Pu,N,Niter);    
  ...
  for(ib=0;ib<nb;ib++)     //bucle externo para barrer en b
    {
      ...
      /*sprintf(nombre,"pk_N%d_alfa%.3lf.txt",N,alfa);
	escribe=fopen(nombre,"wt");
	escribe2=fopen("comprobar.txt","wt");*/
      ...
      while(ro<=ro_max)     //bucle para barrer en rho
	{
	  ...
	  sprintf(file8,"Histogr_Times_b%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.txt",b,alfa,Su,Pu,N,Niter);
	  fich8=fopen(file8,"wt");
	  fclose(fich8);	
	  
          //Guarda la Distribucion de Frecuencias versus K: un histogr. para cada valor de rho y de b
	  sprintf(file9,"Frecs_k_b%.2lf_alfa%.1lf_S%.2lf_P%.2lf_N%d_%diter.txt",b,alfa,Su,Pu,N,Niter);
