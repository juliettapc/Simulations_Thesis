...
sprintf(file2,"medias/Medias_Smax_y_Tpos_vs_Q_tpo_N%d_f%d_ER_%diter.dat",N,f,Niter);
  fich2=fopen(file2,"at");
  fclose(fich2);


  sprintf(file,"medias/Individuales_Smax_vs_Q_N%d_f%d_ER_%diter.dat",N,f,Niter);      //en modo "at" escribo sin borrar lo que ya este escrito
  fich=fopen(file,"at");
  fclose(fich);


  sprintf(file4,"medias/Individuales_tpo_vs_Q_N%d_f%d_ER_%diter.dat",N,f,Niter);      //en modo "at" escribo sin borrar lo que ya este escrito
  ...
  for(iter=1;iter<=Niter;iter++)
  {
      ...
      /////////  LECTURA DEL FICHERO  ////////      
        sprintf(file3,"Consenso_N%d_Q%d_f%d_ER_iter%d.dat",N,Q,f,iter);   // para las simus de Q>=220
