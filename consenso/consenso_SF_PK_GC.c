...
//2-7-09: calculo la pk de la GC de consenso y por capas
...
# define multiplo_PK_GC  50 //(cada cuantos contadores escribe la pk_GC)
...
char file[256],file3[256];// file1[256],file2[256],,file4[256],file5[256],file6[256],file7[256],file8[256],
char file9[256],file12[256],file13[256];
...
int  i, j, jj,  w,g, d, q, x[N+1],y,z, C[N+1][N+1],n, ValorA, steps,s,flat,C_GC[N+1][N+1];
...
double PK_GC[N+1][f+1]; //la posicion rasgo=0 es para el consenso tot
...
void histograma_PK_GC();


FILE *fich, *fich3;//,*fich1,*fich2,*fich4,*fich5,*fich6,*fich7,*fich8,
FILE *fich9,*fich12,*fich13;
...
//	average_path_length_topol();
	...
	{				
		...
		if(((contador % multiplo_PK_GC)==0.0) || (flag==1))   
		  {
		    ...
		    sprintf(file12,"Pk_GC_tot_N%d_Q%d_f%d_SF_%diter_%dtpo.dat",N,Q,f,Niter,contador);
		...
		if(((contador % multiplo_PK_GC)==0.0) || (flag==1) )   
		  {		    
		    ...
		    sprintf(file13,"Pk_rasgos_N%d_Q%d_f%d_SF_%diter_%dtpo.dat",N,Q,f,Niter,contador);
			...
			for(rasgo=1;rasgo<=f;rasgo++)
			  {
			    fprintf(fich13,"    %f",PK_GC[i][rasgo]);	
			  }
		...
		printf("S_max(t=%d)=%d  n_links_bl=%d  n_links_S1=%d  N_cl=%d  \n\n",contador,S_max,n_links_blocked,n_links_S1,NCLUSTERS);
		...
		contador++;		
	      ...
	      }//fin de la condicional de calculo y escritura
...
void find_Smax()    //basado en la rutina TopologiaCP 
...
{                               //si rasgo=0, estoy calculando la pk de la gc de consenso tot.
  ...
  for(ii=1;ii<=GComp;ii++)
    {
      ...
      PK_GC[k_GC[i]][rasgo]++;     //ojo!! utilizo no la k[i] ni la NN[], sino la k_GC[]!!!
  ...
  for(i=1;i<=GComp;i++)
    {
      PK_GC[i][rasgo] = PK_GC[i][rasgo]/aux;
