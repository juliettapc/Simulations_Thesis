...
//Guarda S_max, <Sf_max> y Sf_max[rasgo] en cada paso de tiempo
	sprintf(file,"Consenso_N%d_Q%d_f%d_ER_iter%d_layers2.dat",N,Q,f,iter);
	fich=fopen(file,"wt");
	fclose(fich);
...
/*	sprintf(file8,"Av_path_length_N%d_Q%d_f%d_ER_iter%d.dat",N,Q,f,iter);
	fich8=fopen(file8,"wt");
	fclose(fich8);  */

//Guarda el <CC> de la GC de consenso tot y por capas
	sprintf(file9,"CC_N%d_Q%d_f%d_ER_iter%d_CC_layers2.dat",N,Q,f,iter);
	...
	{				
		...
		if(((contador % multiplo_PK_GC)==0.0) || (flag==1))   
		  {
		    ...
		    sprintf(file12,"Pk_GC_tot_N%d_Q%d_f%d_ER_%diter_%dtpo.dat",N,Q,f,Niter,contador);
		...
		if(((contador % multiplo_PK_GC)==0.0) || (flag==1))   
		  {		    
		    ...
		    sprintf(file13,"Pk_rasgos_N%d_Q%d_f%d_ER_%diter_%dtpo.dat",N,Q,f,Niter,contador);
