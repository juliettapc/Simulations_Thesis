...
# define columnas  12    //que contiene el archivo de entrada (12 el de Ncl,    14 el de S_max, Av_p_l y CCoef)    


# define promediar 3     // el número de la columna del archivo de entrada a partir de la cual empiezan los datos
...
float M_renorm[filas+1][4];
double Media_bines[Nbines],norma[Nbines],delta_tau=0,aux_indice;
float Media[filas+1],Magnitud_tot_bines[Nbines+1],Magnitud_tot[filas+1],valor_final, media_valor_final,valor_final_rasgos, media_valor_final_rasgos;
...
//	  sprintf(file6,"medias/Renorm_CC_vs_tpo_N%d_Q%d_f%d_SF_iter%d.dat",N,Q,f,iter);
	...
	sprintf(file6,"medias/Renorm_N_Cl_vs_tpo_N%d_Q%d_f%d_SF_iter%d.dat",N,Q,f,iter);
...
//imprimo el fichero de los bines
	...
	fprintf(fich4,"%f    %f    %f",aux_indice+(delta_tau/2.0),Media_bines[j],Magnitud_tot_bines[j]); 	 //la posicion en x es el centro de cada intervalo
    ...
    media_valor_final_rasgos=media_valor_final_rasgos/Niter;
    media_valor_final=media_valor_final/Niter;
    
    


    fich4=fopen(file4,"at");
    fprintf(fich4,"1.00    %f    %f",media_valor_final_rasgos,media_valor_final);         //añado a mano el valor final
    fclose (fich4);
