...
# define  k_star  800      //para el targetted
...
// Calculo las probabilidades de cambio de estrat para cada clase de conectividad:  imitacion incondicional
	...
	for(i=k_o;i<=K_max;i++)
	{
	    ...
	    k_inf=T*k;     //limite inferior del sumatorio

	    P_dc[i]=0.0;
	    P_cd[i]=0.0;
	    
	    for (kk=k_inf;kk<=K_max;kk++)    //sumatorio en k'
	    {
		
		
		P_dc[i]+=kk*Pk[kk]*Ck[kk]/k_media;
		P_cd[i]+=kk*Pk[kk]*(1.0-Ck[kk])/k_media;
