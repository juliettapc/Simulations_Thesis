...
# define  k_star  10      //para el targetted

# define t_fin 1000
# define gamma 3.0      //  exponente de la power law  

# define rho   0.9    // concentracion inicial  (para cuando no targetted)
...
// Calculo las probabilidades de cambio de estrat para cada clase de conectividad
	...
	for(i=k_o;i<=K_max;i++)
	{
		...
		beta=T*max;          //        ojo! válido sólo para P=S=0; R=1   (PD) 
		beta=1.0/beta;
		
		tita_cd=PayC[kk]-PayD[i];
		tita_dc=PayD[kk]-PayC[i];
		...
		P_dc[i]+=kk*Pk[kk]*beta*tita_cd*Ck[kk]/k_media;
		P_cd[i]+=kk*Pk[kk]*beta*tita_dc*(1.0-Ck[kk])/k_media;
