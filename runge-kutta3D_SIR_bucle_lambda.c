...
# define lambda_ini   0.00  //tasa de infeccion (como es una prob., va de 0 a 1)
# define lambda_fin   1.01  
# define delta_lambda   0.01 
...
int main()
{
  ...
  sprintf(file2,"SIR_lambda%.2lf-%.2lf_mu%.2lf_k%d.dat",lambda_ini,lambda_fin,mu,k);   
  ...
  while(lambda<=lambda_fin)
	...
	{                  //k_*[i=2]  corresponde con las k1,k2,k3,k4
      ...
      sprintf(file1,"Evol_tepm_SIR_lambda%.2lf_mu%.2lf_k%d_rho_ini%lf.dat",lambda,mu,k,x_old[2]);   
      ...
      while(t<=t_final)
	{
      ...
      fprintf(escribe2,"%lf  %lf  %lf  %lf  %lf  %d\n",lambda,x_old[1],x_old[2],x_old[3],mu,k); 
      fclose(escribe2);
    ...
    }   //fin del bucle barrer en lambda
