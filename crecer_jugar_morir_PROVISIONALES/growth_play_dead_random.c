...
int main()
{
  ...
  printf("\n\nPrograma Growth & Play & Dead (random) (%d iter)\nN=%d  b_ini=%lf b_fin=%lf eps=%lf  TauT=%d  TauD=%d\n", Niter,N,b_min,b_max,eps,tauT,tauD);
  
  
  
  
  
  sprintf(file1,"C_vs_b_N%d_%d-%d-b_ini%.2lf-e%.2lf_tpo%d_%diter_rand.dat",N,tauT,tauD,b_min,eps,jugadas_evolucion,Niter);  
  ...
  while(b<=b_max)     //bucle externo para barrer en b
    {
      ...
      sprintf(file2,"Pk_Pkc_N%d_%d-%d-b%.2lf-e%.2lf_tpo%d_%diter_rand.dat",N,tauT,tauD,b,eps,jugadas_evolucion,Niter);      
      ...
      sprintf(file3,"EvolCoop_N%d_%d-%d-b%.2lf-e%.2lf_tpo%d_%diter_rand.dat",N,tauT,tauD,b,eps,jugadas_evolucion,Niter);    // id.    
      fich3=fopen(file3,"wt");
      fclose(fich3);
      
      
      
      sprintf(file4,"Conex_N%d_%d-%d-b%.2lf-e%.2lf_tpo%d_%diter_rand.dat",N,tauT,tauD,b,eps,jugadas_evolucion,Niter);      
      ...
      for(iter=1;iter<=Niter;iter++)         //inicio bucle de estadistica
	{
	  ...
	  do{
	    ...
	    for(II=1;II<=tauT;II++)
	      {	
		muerte_nodo();   // para mantener el tamaño cte --->> hueco=...	
		// y   recalculo Probabilidad de attachment		
...
{               //   junto con todas sus conexiones
  ...
  ben[hueco]=0;   // para que calcule bien las nuevas prob de attachment
  P[hueco]=0;

  for(i=hueco+1;i<=N;i++)     //desplazo todos los intervalos
    {
      P[i]=P[i]-fit[hueco];
    }
...
void nuevo_nodo_hueco()  //  "hueco" es el indice del nodo nuevo
{
  ...
  norma2=norma;
       //ya he excluido al propio nodo recien llegado al matar al "hueco"
